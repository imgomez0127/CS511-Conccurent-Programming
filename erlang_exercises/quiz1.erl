...
is_valid(TownR) ->
    lists:all(
        fun(Person) ->
            not has_cycle(TownR,[],[Person]) 
        end,
        maps:keys(TownR)
    ). 

has_been_visited(Visited,Children)->
    lists:any(
        fun(Person) -> 
            lists:member(Person,Visited)
        end,
        Children).
...
has_cycle(TownR,Visited,Current) ->
    DD = lists:flatten(lists:map(fun (Id) -> childrenOf(Id,TownR) end, Current)),
    case has_been_visited(Visited,Current) of
        true -> true;
        false -> has_cycle(TownR,Visited++Current,DD)
    end.
